...
async function startBotProcess() {
  ...
  if (!token) {
    console.error("❌ FEHLER: DISCORD_TOKEN ist nicht in den Secrets hinterlegt!");
    return;
  }

  console.log(`ℹ️ Starte Bot... (Token Länge: ${token.length})`);
  ...
  try {
    ...
    if (err.message.includes("invalid token")) {
      console.error("   Der Token wird von Discord abgelehnt.");
      console.error("   Grund: Entweder falsch kopiert oder durch Posten im Chat gesperrt.");
    } else if (err.message.includes("privileged intent")) {
      console.error("   Berechtigungs-Fehler (Intents).");
      console.error("   Bitte im Dev Portal 'Message Content Intent' aktivieren.");
    } else {
...
async function handleModal(interaction: ModalSubmitInteraction) {
  ...
  if (interaction.customId === "modal_suggestion") {
    ...
    console.log(`Application submitted: ${title} by ${interaction.user.tag}`);
  }
...
export function startBot() {
  // Startbot wurde oben bereits über startBotProcess aufgerufen
  // Diese Export-Funktion bleibt für Kompatibilität mit routes.ts erhalten
}
